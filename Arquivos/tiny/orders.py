...
class TinyOrderService:
    ...
    def __init__(self, client: Optional[TinyClient] = None, config: Optional[TinyConfig] = None):
        self.config = config or TinyConfig()
        self.client = client or TinyClient(config=self.config)

    def buscar_pedidos_recentes(self, dias_atras: Optional[int] = None) -> List[PedidoTiny]:
    ...
    def executar_fluxo_extracao(self, caminho_saida: Optional[str] = None) -> bool:
